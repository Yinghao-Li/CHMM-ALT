...
class DirCHMMMetric(Metric):
    def __init__(self, conc_o2o=None, conc_e2e=None):
        super(DirCHMMMetric, self).__init__()
        self.conc_o2o = conc_o2o
        self.conc_e2e = conc_e2e


class NeuralDirModule(nn.Module):
    def __init__(self,
                 config: DirCHMMConfig):
        super(NeuralDirModule, self).__init__()
        ...
        self._use_dirichlet_emiss = config.use_dirichlet_emiss
        ...
        if self._use_dirichlet_emiss:
            ...
            self._emission_2o = self._emission_2e = self._softmax = self._sigmoid = None
            self._concentration_base = self._concentration_range = 0
    ...
    def forward(self,
        ...
        if self._use_dirichlet_emiss:
            ...
            diric_params[:, :, self._e2e_diag_idx] = \
                entity_emiss_diag(conc_e2e_scale).repeat_interleave(2, dim=-1)
            ...
            non_diag_values = entity_emiss_nondiag(
                x=conc_e2e_scale, n_lbs=self._d_obs, tp=turning_point, exp_term=self._nondiag_exp
            ).detach()
            ...
            conc_o2o = conc_e2e_scale = 0
            dirichlet_emiss = None
        return nn_trans, dirichlet_emiss, (conc_o2o, conc_e2e_scale)
...
class DirCHMM(nn.Module):
    ...
    def __init__(self,
        ...
        self._nn_module = NeuralDirModule(config)
    ...
    def _initialize_states(self,
        ...
        nn_trans, nn_emiss, concentration = self._nn_module(embs)
        ...
        if nn_emiss is not None:
            ...
            if self._inter_results.conc_o2o is None:
                self._inter_results.conc_o2o = concentration[0].detach().cpu().numpy()
                self._inter_results.conc_e2e = concentration[1].detach().cpu().numpy()
            else:
                self._inter_results.conc_o2o = np.r_[self._inter_results.conc_o2o,
                                                     concentration[0].detach().cpu().numpy()]
                self._inter_results.conc_e2e = np.r_[self._inter_results.conc_e2e,
                                                     concentration[1].detach().cpu().numpy()]
        ...
        # TODO: substitute the "sum" function by Dirichlet sampling
