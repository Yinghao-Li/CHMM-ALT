...
from seqlbtoolkit.chmm.dataset import CHMMDataset, collate_fn

from label_model.dir_chmm.train import DirCHMMTrainer
from label_model.dir_chmm.args import DirCHMMArguments, DirCHMMConfig

logger = logging.getLogger(__name__)


def chmm_train(args: DirCHMMArguments):
    # setup logging
    set_logging(log_dir=args.log_dir)

    logging_args(args)
    set_seed(args.seed)
    config = DirCHMMConfig().from_args(args)
    ...
    result_file = os.path.join(args.output_dir, 'dir-chmm-results.txt')
    ...
    with open(result_file, 'w') as f:
        ...
        if test_metrics is not None:
            ...
            f.write("\n")
    inter_result_file = os.path.join(args.output_dir, 'inter-results.pt')
...
if __name__ == '__main__':
    ...
    # --- set up arguments ---
    parser = HfArgumentParser(DirCHMMArguments)
