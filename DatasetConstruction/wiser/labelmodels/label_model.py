...
class ClassConditionalLabelModel(LabelModel):
    ...
    def _get_labeling_function_likelihoods(self, votes, device):
        """
        ...
        cll = torch.zeros(votes.shape[0], self.num_classes, device=device)

        # Initializes normalizing constants
        z_prop = self.propensity.unsqueeze(1)
        z_prop = torch.cat((z_prop, torch.zeros_like(z_prop)), dim=1)
        ...
        for i, j, v in zip(votes.row, votes.col, votes.data):
            for k in range(self.num_classes):
                if v == (k + 1):
                    ...
                    logp = self.propensity[j] - self.accuracy[j, k] - z_acc[j, k]
                    logp -= torch.log(torch.tensor(self.num_classes - 1.0, device=device))
                    cll[i, k] += logp
