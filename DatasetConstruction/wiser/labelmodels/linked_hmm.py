...
class LinkedHMM(ClassConditionalLabelModel):
    ...
    def __init__(self, num_classes, num_labeling_funcs, num_linking_funcs,
                 init_acc=.9, acc_prior=1, balance_prior=1, device='cpu'):
        """Constructor.
    ...
    def forward(self, label_votes, link_votes, seq_starts):
        ...
        """
        jll = self._get_labeling_function_likelihoods(label_votes, self.device).to(self.device)
        link_cll = self._get_linking_function_likelihoods(link_votes).to(self.device)
        norm_start_balance = self._get_norm_start_balance().to(self.device)
        norm_transitions = self._get_norm_transitions().to(self.device)
    ...
    def get_most_probable_labels(self, label_votes, link_votes, seq_starts):
        ...
        for label_votes, link_votes, seq_starts in self._create_minibatches(
            ...
            jll = self._get_labeling_function_likelihoods(label_votes, device=self.device)
    ...
    def get_label_distribution(self, label_votes, link_votes, seq_starts):
        ...
        for label_votes, link_votes, seq_starts in self._create_minibatches(
            ...
            label_cll = self._get_labeling_function_likelihoods(label_votes, device=self.device)
            link_cll = self._get_linking_function_likelihoods(link_votes)
    ...
    def _get_linking_function_likelihoods(self, votes):
        ...
        cll = torch.zeros((votes.shape[0], self.num_classes, self.num_classes), device=self.device)

        # Initializes normalizing constants
        z_prop = self.link_propensity.unsqueeze(1)
        z_prop = torch.cat((z_prop, torch.zeros((self.num_linking_funcs, 1), device=self.device)), dim=1)
