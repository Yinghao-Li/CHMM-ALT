...
class Parser:
    ...
    def __init__(self, model, correct_entities=True):
        ...
        if correct_entities:
            self.nlp.add_pipe(correct_entities, after='ner')
...
def _correct_tokenisation(parsed):
    ...
    while tokens_stack:
        ...
        match = re.match(r"(%s)([\d.,]+.*)" % "|".join(CURRENCY_CODES), token.text)
        if match:
        ...
        match = re.match(r"([\d.,]+)(bl?n?\.?|ml?n?\.?|k\.+)$", token.text, re.IGNORECASE)
        if match:
            tokens += [match.group(1), match.group(2)]
            spaces += [False, bool(token.whitespace_)]
            has_changed = True
...
def _correct_parsing(parsed):
    ...
    for token in parsed:
        ...
        elif re.match(r"(?:bl?n?\.?|ml?n?\.?|k\.+)$", token.text, re.IGNORECASE):
            token.pos = spacy.symbols.NUM
            token.tag = parsed.vocab.strings["CD"]
            if token.dep_ == "compound":
                token.dep = parsed.vocab.strings["nummod"]
        elif (token.text == "won" and token.i > 1 and
              (token.nbor(-1).text[0].isdigit() or token.nbor(-1).text.lower() in
               {"million", "billion", "mln", "bln", "bn", "thousand", "m", "k", "b", "m.", "k.", "b.",
                "mln.", "bln.", "bn."})):
            token.pos = spacy.symbols.PROPN
            token.tag = parsed.vocab.strings["NNP"]
...
def correct_entities(parsed, recursive=True):
    ...
    for ent in existing_ents:
        ...
        elif (ent.end < len(parsed) and parsed[ent.end].lemma_.lower() in {"million", "billion", "mln", "bln", "bn",
                                                                           "thousand",
                                                                           "m", "k", "b", "m.", "k.", "b.", "mln.",
                                                                           "bln.", "bn."}
              and ent.label in {spacy.symbols.MONEY, spacy.symbols.CARDINAL}):
        ...
        elif parsed[ent.end - 1].lower_.rstrip(".") in LEGAL_SUFFIXES and (
                ent.end > ent.start + 1) and ent.label != spacy.symbols.ORG:
    ...
    for token in parsed:
        if (token.text in CURRENCY_CODES | CURRENCY_SYMBOLS and token.ent_type != spacy.symbols.MONEY
            ...
            for i in range(token.i + 2, len(parsed)):
                if any([start <= i < end for start, end, _ in new_ents]):
                    entity_end = i + 1
                else:
                    break
    ...
    if has_changed:
        ...
        if recursive:
            return correct_entities(parsed, False)
    return parsed
