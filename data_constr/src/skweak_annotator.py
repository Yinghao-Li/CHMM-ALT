...
WIKIDATA_SMALL = os.path.join(os.path.dirname(__file__), "../Dependency/wikidata_small_tokenised.json")
COMPANY_NAMES = os.path.join(os.path.dirname(__file__), "../Dependency/company_names_tokenised.json")
GEONAMES = os.path.join(os.path.dirname(__file__), "../Dependency/geonames.json")
CRUNCHBASE = os.path.join(os.path.dirname(__file__), "../Dependency/crunchbase.json")
PRODUCTS = os.path.join(os.path.dirname(__file__), "../Dependency/products.json")
FIRST_NAMES = os.path.join(os.path.dirname(__file__), "../Dependency/first_names.json")
FORM_FREQUENCIES = os.path.join(os.path.dirname(__file__), "../Dependency/form_frequencies.json")
...
class CoNLL2003Annotator(CombinedAnnotator):
    ...
    def add_models(self):
        ...
        self.add_annotator(ModelAnnotator("BTC", os.path.join(os.path.dirname(__file__), "../Dependency/btc")))
        self.add_annotator(TruecaseAnnotator(
            "BTC_truecase", os.path.join(os.path.dirname(__file__), "../Dependency/btc"), FORM_FREQUENCIES
        ))
...
class WikiGoldAnnotator(CoNLL2003Annotator):
    def add_all(self):
        """Adds all implemented annotation functions, models and filters"""
    ...
    def add_models(self):
        ...
        self.add_annotator(ModelAnnotator(
            "CoNLL2003", os.path.join(os.path.dirname(__file__), "../Dependency/conll2003")))
        self.add_annotator(TruecaseAnnotator(
            "CoNLL2003_truecase", os.path.join(os.path.dirname(__file__), "../Dependency/conll2003"), FORM_FREQUENCIES
        ))
    ...
    def __call__(self, doc):
        ...
        for source in doc.spans:
            ...
            for span in doc.spans[source]:
                ...
                elif span.label_ in {"PER", "ORG", "LOC", "MISC"}:
...
class SentBoundary:
    def __init__(self, start_idx):
        self.start_indices = start_idx[:-1]
...
@Language.factory("sent_bd", default_config={"start_idx": []})
def sent_boundary(nlp, name, start_idx):
    return SentBoundary(start_idx)


def construct_spacy_doc(sents: List[List[str]], spacy_model):
    # construct sentences
    s_tokens = []
    for s in sents:
        s_tokens += s
    sent = ' '.join([' '.join(s) for s in sents])
    sep_sent = ' '.join([' '.join(s + ['@SEP@']) for s in sents])

    # helper document
    if 'sent_bd' in spacy_model.pipe_names:
        spacy_model.remove_pipe('sent_bd')
    ...
    spacy_model.add_pipe('sent_bd', config={"start_idx": start_indices}, before="parser")
