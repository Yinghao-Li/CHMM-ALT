...
class CDRDatasetReader(DatasetReader):
    ...
    def __init__(self, token_indexers: Dict[str, TokenIndexer] = None, use_regex: bool = True) -> None:
        ...
        if use_regex:
            infix_re = compile_infix_regex(self.nlp.Defaults.infixes + [r'-'] + [r'[/+=\(\)\[\]]'])
            prefix_re = compile_prefix_regex(self.nlp.Defaults.prefixes + [r'[\'\(\[]'])
            suffix_re = compile_suffix_regex(self.nlp.Defaults.suffixes + [r'[\.\+\)\]]'])
    ...
    def text_to_instance(self, tokens: List[Token], tags: List[str] = None,
                         sentence_spans: List[Tuple[int, int]] = None) -> Instance:
